...
;Load LastStep
FileReadLine, LastStepPH, config.ini, 2
StringTrimLeft, LastStep, LastStepPH, 11
...
SaveProgress:
file := FileOpen("Config.ini", "rw")

Loop
{
	If(A_Index == 2)
	{
		file.WriteLine("LastStep = "LastStep)
	} else if(A_Index == 3)
	{
		file.Writeline("Act = "Act)
	}else {
		Line := file.ReadLine()
	}
	If !Line
